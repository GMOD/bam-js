...
- **Lines 368-369**: Moved `dpositions` and `cpositions.length` checks outside
  hot loops
...
- **Line 96**: Changed `String.fromCharCode(b1, b2)` to
  `String.fromCharCode(b1) + String.fromCharCode(b2)`
...
Due to ±7-33% measurement variance, end-to-end improvements are hard to quantify
precisely. Profiling data is more reliable.

**Conservative estimate**: 5-15% faster for typical workloads **Cache
benefits**: 2-3x for overlapping/repeated queries (preserved from original)
...
3. **WebAssembly for sequence decoding**: Would require major effort but could
   improve the #1 hotspot
4. **Profile before further optimization**: Use CPU profiling, not wall-clock
   benchmarks

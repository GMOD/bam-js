...
**The range cache does not rescue it.** It joins only _contiguous_ runs, so a
...
Swept while the harness existed. Format is
`bare reads / bare MB, cache req / cache MB`:
...
so it is less likely to sit _wholly_ past the query, and the batch-and-stop from

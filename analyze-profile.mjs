...
console.log('\n\nBam-js specific functions:')
const bamJsFuncs = summary.filter(s =>
  s.name.includes('record.js') ||
  s.name.includes('bamFile.js') ||
  s.name.includes('util.js') ||
  s.name.includes('bai.js') ||
  s.name.includes('bgzf')
)

for (const func of bamJsFuncs.slice(0, 20)) {
  console.log(`${func.percentage.toFixed(2).padStart(6)}% | ${func.name}`)
}
